...
show_start_setup() {
	if [ "$auto" = 0 ]; then
		...
		echo
		echo '使用默认选项继续安装WireGuard'
	fi
}

enter_server_address() {
	echo
	printf "你想使用域名连接这台WireGuard服务器吗？ [y/N] "
	read -r response
	case $response in
		[yY][eE][sS]|[yY])
			use_dns_name=1
	...
	if [ "$use_dns_name" = 1 ]; then
		...
		ip="$server_addr"
		echo
		echo "Note: 请确认该域名已解析到这台WireGuard服务器的IPv4地址"
	else
		detect_ip
		check_nat_ip
	fi
...
new_client_setup() {
	...
	if [ "$1" = "add_client" ]; then
		...
		if [[ ! "$specify_ip" =~ ^[yY]$ ]]; then
			echo "使用自动分配的IP地址 192.168.0.$octet."
		fi
	fi
	if [[ "$specify_ip" =~ ^[yY]$ ]]; then
		echo
		...
		until [[ $client_ip =~ ^192\.168\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]] \
			...
			if [[ ! $client_ip =~ ^192\.168\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]]; then
				echo "IP地址无效. IP地址范围：192.168.0.2 - 192.168.0.254."
			else
				echo "该IP地址已被使用，请选择其它IP地址."
			fi
			read -rp "请输入该客户端的内网IP地址(例如：192.168.0.X): " client_ip
			octet=$(printf '%s' "$client_ip" | cut -d "." -f 4)
		done
...
PresharedKey = $psk
AllowedIPs = 192.168.0.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = 192.168.0.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd -q --zone=trusted --add-source=192.168.0.0/24
		firewall-cmd -q --permanent --add-port="$port"/udp
		firewall-cmd -q --permanent --zone=trusted --add-source=192.168.0.0/24
		# Set NAT for the VPN subnet
		firewall-cmd -q --direct --add-rule ipv4 nat POSTROUTING 0 -s 192.168.0.0/24 ! -d 192.168.0.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 192.168.0.0/24 ! -d 192.168.0.0/24 -j MASQUERADE
		if [[ -n "$ip6" ]]; then
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 192.168.0.0/24 ! -d 192.168.0.0/24 -j MASQUERADE
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 192.168.0.0/24 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 192.168.0.0/24 ! -d 192.168.0.0/24 -j MASQUERADE
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 192.168.0.0/24 -j ACCEPT
		...
		5)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					...
					firewall-cmd -q --zone=trusted --remove-source=192.168.0.0/24
					firewall-cmd -q --permanent --remove-port="$port"/udp
					firewall-cmd -q --permanent --zone=trusted --remove-source=192.168.0.0/24
					firewall-cmd -q --direct --remove-rule ipv4 nat POSTROUTING 0 -s 192.168.0.0/24 ! -d 192.168.0.0/24 -j MASQUERADE
					firewall-cmd -q --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 192.168.0.0/24 ! -d 192.168.0.0/24 -j MASQUERADE
